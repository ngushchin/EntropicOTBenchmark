...
#     current_grad = torch_grad(current_state, current_log_p, is_grads_batched=True)
...
#     proposal_grad = torch_grad(proposal_state, proposal_log_p, is_grads_batched=True)
    
    with torch.no_grad():        
#         current_log_q = -0.25*torch.norm(proposal_state - current_state - delta_time*current_grad, p=2, dim=-1).square()
#         proposal_log_q = -0.25*torch.norm(current_state - proposal_state - delta_time*proposal_grad, p=2, dim=-1).square()
